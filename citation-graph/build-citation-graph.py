...
def create_paper_node(session, obj):
    ...
    try:
        ...
        result = session.run(
            "MATCH (n:paper {title: $title}) RETURN n", title=obj.get("title")
        )
        if result.single() is not None:
            logging.info(
                f"⏩ Skipping title because it is already there: {obj['title']}"
            )
            return

        authors = [
            author["lastName"]
            for author in obj["creators"]
            if author["creatorType"] == "author"
        ]
        author_tag = (
            authors[0]
            if len(authors) == 1
            else " und ".join(authors)
            if len(authors) == 2
            else authors[0] + " et al."
        )
        tags = [tag["tag"] for tag in obj["tags"] if not tag["tag"].startswith("round")]
        round = [
            tag["tag"][6:] for tag in obj["tags"] if tag["tag"].startswith("round")
        ]
        ...
        result = session.run(
            ...
            authors=authors,
            doi=obj.get("DOI") or obj.get("note", "").replace("DOI: ", "")
            if ("DOI" in obj or "note" in obj)
            else None,
            round=round,
            tags=tags,
